...
Avoid over-penalizing answers for surface-level differences if they preserve the meaning and ordering of actions.
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Gemini Inference Script")
    parser.add_argument("--input_json", type=str, required=True, help="Path to the input JSON file")
    parser.add_argument("--remove_special_tokens", action='store_false', help="Remove special tokens from the ground truth qa query")

    args = parser.parse_args()
    result_json_path = args.input_json
    new_path = result_json_path.replace(".json", "_aftergemini_actionplanningflash2.json")
    ...
    for idx in range(start_idx, len(data)):
        ...
        if qa_item['metadata']['question_type'] != 'action_planning':
            print(f"Question type mismatch: {qa_item['metadata']['question_type']} != action_planning")
            continue
