...
class PointnetSAModuleVotes(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor,
        ...
        num_nans = torch.isnan(new_features).sum()
        num_infs = torch.isinf(new_features).sum().item()
        tensor_size = new_features.numel()

        # from llava.utils import rank0_print

        print(f"Number of NaNs: {num_nans}")
        print(f"Number of Infs: {num_infs}")
        print(f"Total tensor size: {tensor_size}")

        valid_mask = torch.isfinite(new_features)  # Excludes NaN and Inf

        # Extract valid values
        valid_values = new_features[valid_mask]   

        # Shuffle and randomly sample 10 values
        num_samples = min(10, valid_values.numel())  # Ensure we don't sample more than available
        random_indices = torch.randperm(valid_values.numel())[:num_samples]

        # Print the sampled values
        print("Randomly sampled valid values:", valid_values[random_indices].tolist())
