...
class AlignerPoint3DLoader(torch.utils.data.Dataset):
    '''Dataloader for 3D points and labels.'''
    ...
    def __init__(self, datapath_prefix='data', voxel_size=0.05,
        ...
        if split is None:
            split = ''
        # Find all aligned vertex .npy files and strip the suffix to get base path
        # self.data_paths = sorted(glob(join(datapath_prefix, split, '*_aligned_vert.npy')))
        self.data_paths = sorted(glob(join(datapath_prefix, '*', '*_aligned_vert.npy')))
        self.data_paths = [p.replace('_aligned_vert.npy', '') for p in self.data_paths]
        if len(self.data_paths) == 0:
            raise Exception('No .npy files found in the specified datapath.')
    ...
    def __getitem__(self, index_long):
        index = index_long % len(self.data_paths)
        prefix = self.data_paths[index]

        locs_in = np.load(prefix + '_vert.npy')[:, :3]
        feats_in = np.load(prefix + '_vert.npy')[:, 3:6] if self.input_color else np.zeros_like(locs_in)
        labels_in = np.load(prefix + '_sem_label.npy')
        labels_in[labels_in == -100] = 255
        labels_in = labels_in.astype(np.uint8)

        locs = self.prevoxel_transforms(locs_in) if self.aug else locs_in
        print(f'before voxelizer: locs.shape: {locs.shape}, feats_in.shape: {feats_in.shape}, labels_in.shape: {labels_in.shape}')
        locs, feats, labels, inds_reconstruct, _ = self.voxelizer.voxelize(locs, feats_in, labels_in)
        print(f'after voxelizer: locs.shape: {locs.shape}, feats_in.shape: {feats.shape}, labels_in.shape: {labels.shape}')
        ...
        feats = torch.from_numpy(feats).float() / 127.5 - 1. if self.input_color else torch.ones(coords.shape[0], 3)
        labels = torch.from_numpy(labels).long()
