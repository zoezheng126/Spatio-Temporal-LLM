...
import collections
import numpy as np
from llava.model.openscene.dataset.voxelization_utils import sparse_quantize
...
class Voxelizer:
    ...
    def voxelize(self, coords, feats, labels=None, center=None, link=None, return_ind=False):
        ...
        print(f'125 voxelizer min_coords: {min_coords}')
        M_t = np.eye(4)
        M_t[:3, -1] = -min_coords
        rigid_transformation = M_t @ rigid_transformation
        coords_aug = np.floor(coords_aug - min_coords)
        print(f'127 voxelizer.py coords_aug: {coords_aug.shape}')
        ...
        if labels is not None:
            ...
            coords_aug, feats, labels = coords_aug[inds], feats[inds], None

        # Normal rotation
        if feats.shape[1] > 6:
            feats[:, 3:6] = feats[:, 3:6] @ (M_r[:3, :3].T)

        print(f'142 voxelizer: {coords_aug.shape}, feat.shape: {feats.shape}')

        if return_ind:
            print('return_ind')
            return coords_aug, feats, labels, np.array(inds_reconstruct), inds, rigid_transformation
        if link is not None:
            print('return link')
            return coords_aug, feats, labels, np.array(inds_reconstruct), link[inds], rigid_transformation

        return coords_aug, feats, labels, np.array(inds_reconstruct), rigid_transformation, min_coords
    ...
    #     rigid_transformation = np.eye(4)   # ! hardcode
    #     homo_coords = np.hstack((coords, np.ones((coords.shape[0], 1), dtype=coords.dtype)))
    #     coords_aug = np.floor(homo_coords @ rigid_transformation.T[:, :3])  # !
    ...
    #     print(f'127 voxelizer.py coords_aug: {coords_aug.shape}')
    #     inds, inds_reconstruct = sparse_quantize(coords_aug, return_index=True)
    #     print(f'129 voxelizer.py inds max: {inds.max()}, feats.shape: {feats.shape}, labels.shape: {labels.shape}')

    #     if inds.max() >= labels.shape[0]:
    #         coords_aug, feats = coords_aug[inds], feats[inds]
    #         labels = np.ones(feats.shape[0], dtype=labels.dtype)
    #     else:
    #         coords_aug, feats, labels = coords_aug[inds], feats[inds], labels[inds]
